...
export function isPlaceholderAoiName(name) {
  ...
  // Match fallback strings like "AOI 13.04N, 80.25E"
  if (/^aoi\s+\d+\.\d+[ns],\s*\d+\.\d+[ew]$/.test(normalized)) return true;
  return false;
}
...
export async function resolveAoiName(geo) {
  ...
  try {
    const res = await fetch(
      `https://nominatim.openstreetmap.org/reverse?format=jsonv2&lat=${centroid.lat}&lon=${centroid.lon}&zoom=12&addressdetails=1&email=dashboard-user@epm.local`,
      {
        headers: {
          "Accept-Language": "en",
        },
