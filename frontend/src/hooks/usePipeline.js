...
const STEP_KEYWORDS = {
  ...
  indices:  ["[indices]", "computing spectral indices", "index tifs written",
             ...
             "nbr:", "ndti:", "ndbai:"],
  rpri:     ["[rpri]", "computing rpri", "rpri done", "rpri written"],
  cluster:  ["[cluster]", "gmm fit done", "clustering done", "running gmm", "running fixed absolute thresholds", "cluster 0:", "cluster 1:",
             "cluster tif written"],
...
export function usePipeline() {
  ...
  const handleLogLine = useCallback((rawLine) => {
    ...
    if (inferred) {
      ...
      // Wait, actually, we DO want to auto-advance prior steps because they are strictly sequential!
      setSteps(prev => {
        let changed = false;
        const newState = { ...prev };
        if (newState[inferred] !== "done" && newState[inferred] !== "running") {
          newState[inferred] = "running";
          changed = true;
        }
        const idx = STEP_IDS.indexOf(inferred);
        if (idx !== -1) {
          for (let i = 0; i < idx; i++) {
            if (newState[STEP_IDS[i]] !== "done") {
              newState[STEP_IDS[i]] = "done";
              changed = true;
            }
          }
        }
  ...
  // ─────────────────────────────────────────────────────────────────────────
  const runPipeline = useCallback(async ({ aoi, aoiName, startDate, endDate, maxCloud }) => {
    ...
    const result = {
      pipelineState: "complete", job_id: finalId,
  ...
  const start = useCallback(async ({ aoi, aoiName, startDate, endDate, maxCloud, hasPolygon }) => {
    ...
    await runPipeline({ aoi, aoiName, startDate, endDate, maxCloud });
