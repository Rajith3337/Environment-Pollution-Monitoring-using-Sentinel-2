    ...
    <div style={{
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "8px 10px", display: "flex", flexDirection: "column", gap: 4 }}>
        {history.map((job, i) => {
          ...
          const rpriMean = job.histograms?.RPRI?.mean ?? job.histograms?.RAQI?.mean;
          const rpriColor = rpriMean == null ? "var(--muted)"
            : rpriMean < 0.25 ? "#34d399"
            : rpriMean < 0.5  ? "#fbbf24"
            : rpriMean < 0.75 ? "#fb923c"
            : "#f87171";
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                ...
                {rpriMean != null && (
                  <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                    <div style={{ width: 6, height: 6, borderRadius: "50%", background: rpriColor, boxShadow: `0 0 5px ${rpriColor}` }} />
                    <span style={{ fontFamily: "var(--mono)", fontSize: 9, color: rpriColor }}>{rpriMean.toFixed(3)}</span>
                  </div>
                )}
              </div>
            </button>
...
export default function Dashboard({ results, history, jobId, onExit, onChangeDate }) {
  const [activeTab,     setActiveTab]     = useState("rpri");
  ...
  const handleDownloadPdf = useCallback(async () => {
    ...
    setPdfLoading(true);
    try {
      await downloadPdfReport(displayJob, activeAoiName, history);
    } catch (e) {
      ...
      <div style={{ flex: 1, display: "flex", overflow: "hidden" }}>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "18px 20px" }}>
          {!displayJob ? (
            ...
            <>
              {activeTab === "rpri"     && <RpriTab     results={displayJob} key={displayJob.job_id + activeTab} />}
  ...
  return (
    <div style={{
      ...
      <div style={{ fontFamily: "var(--mono)", fontSize: 10, color: "var(--muted2)", textAlign: "center" }}>
        Run a pipeline to generate<br />RPRI heatmaps and indices
      </div>
    </div>
  );
}
