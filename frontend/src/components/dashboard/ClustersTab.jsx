...
import { CLUSTER_LABELS } from "../../data/constants";
...
function deriveCentroids(histograms) {
  // Prefer real GMM centroids saved by the backend
  const saved = histograms?._meta?.gmm_centroids;
  if (Array.isArray(saved) && saved.length === 5) return saved;

  // Fallback: derive weighted mean within each fixed-threshold band from RPRI histogram
...
/**
 * Normalize raw GMM severity scores (NDBI - NDVI, can be negative) to [0, 1].
 * The cleanest class maps to 0.00, the most polluted maps to 1.00.
 */
function normalizeCentroids(raw) {
  if (!raw || raw.length === 0) return raw;
  const mn = Math.min(...raw);
  const mx = Math.max(...raw);
  if (mn === mx) return raw.map(() => 0.5);
  return raw.map(v => (v - mn) / (mx - mn));
}
...
export default function ClustersTab({ results }) {
  ...
  const rawCentroids  = deriveCentroids(results?.histograms);
  const centroids     = normalizeCentroids(rawCentroids);
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16, animation: "fadeIn 0.35s ease" }}>
      ...
      <div style={{ background: "var(--panel)", border: "1px solid var(--border)", borderRadius: 12, padding: "13px 16px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
          ...
          />
          <span style={{ fontFamily: "var(--mono)", fontSize: 9, color: "var(--muted)", marginLeft: 4 }}>GMM · 5 classes</span>
          {date && (
      ...
      <div style={{ background: "var(--panel)", border: "1px solid var(--border)", borderRadius: 12, padding: 16 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <InfoTooltip
              ...
              position="right"
              width={300}
              body="Each pixel is classified into one of 5 pollution risk classes using GMM clustering on a 5D spectral feature vector (NDVI, NDMI, NDBI, NDWI, NDTI). Cluster boundaries are learned from the data, so they reflect natural breaks in the spectral distribution rather than fixed constants."
              legend={[
                { color:"#22c55e", label:"Very Low Risk",  desc:"Clean, high vegetation health" },
                { color:"#84cc16", label:"Low Risk",       desc:"Minor stress, manageable" },
                { color:"#eab308", label:"Moderate Risk",  desc:"Monitoring recommended" },
                { color:"#f97316", label:"High Risk",      desc:"Industrial / degraded zone" },
                { color:"#ef4444", label:"Critical Risk",  desc:"Urgent intervention needed" },
              ]}
            />
            {jobId && date && (
        ...
        <CogLeafletMap jobId={jobId} date={date} layer="pollution_clusters" height={440} label="GMM · 5 classes" aoiGeoJSON={aoiGeoJSON} aoiName={aoiName} />
        ...
        {CLUSTER_LABELS.map((c, i) => (
          ...
          >
            ...
            <div style={{ fontFamily: "var(--sans)", fontWeight: 800, fontSize: 22, color: c.color, marginBottom: 2 }}>
              {centroids[i] != null ? centroids[i].toFixed(3) : "—"}
            </div>
            <div style={{ fontFamily: "var(--mono)", fontSize: 8, color: "var(--muted)", marginBottom: 10 }}>Severity Index · 0=clean 1=polluted</div>
