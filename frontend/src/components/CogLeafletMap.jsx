 ...
 *   layer       – layer name: "RPRI", "pollution_clusters", "NDVI", etc.
...
export default function CogLeafletMap({ jobId, date, layer, height = 260, label, aoiGeoJSON, aoiName }) {
  ...
  useEffect(() => {
    if (tileRef.current) {
      tileRef.current.setOpacity(showOverlay ? 0.85 : 0);
    }
  }, [showOverlay]);
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      if (mapRef.current && tileRef.current) {
        ...
        tileRef.current = L.tileLayer(tileUrl, { opacity: showOverlay ? 0.85 : 0, maxZoom: 20 }).addTo(mapRef.current);
      ...
      const cogLayer = L.tileLayer(tileUrl, {
        opacity: showOverlay ? 0.85 : 0, maxZoom: 20,
  ...
  useEffect(() => {
    ...
    if (aLayer.getBounds().isValid()) {
      map.fitBounds(aLayer.getBounds(), { padding: [20, 20], maxZoom: 14 });
    }
  }, [aoiGeoJSON, aoiName]);

  return (
    <div style={{ position: "relative", width: "100%", height, borderRadius: 8, overflow: "hidden" }}>
      {/* AOI Name badge moved to bottom left out of the overlay */}
      {aoiName && (
        <div 
          onClick={() => {
            if (mapRef.current && aoiLayerRef.current && aoiLayerRef.current.getBounds().isValid()) {
              mapRef.current.fitBounds(aoiLayerRef.current.getBounds(), { padding: [20, 20], maxZoom: 14 });
            }
          }}
          style={{
            position: "absolute", bottom: 12, left: 12, zIndex: 1000, pointerEvents: "auto",
            background: "rgba(6,8,11,0.85)", border: "1px solid var(--border2)",
            borderRadius: 6, padding: "6px 10px",
            fontFamily: "var(--mono)", fontSize: 10, color: "var(--accent)",
            backdropFilter: "blur(4px)", cursor: "pointer", transition: "all 0.2s"
          }}
          onMouseEnter={e => e.currentTarget.style.background = "rgba(0,240,168,0.15)"}
          onMouseLeave={e => e.currentTarget.style.background = "rgba(6,8,11,0.85)"}
        >
          {aoiName}
        </div>
      )}

      <div ref={containerRef} style={{ width: "100%", height: "100%", background: "#06080b" }} />
      <style>{`
        .aoi-name-label {
      ...
      {jobId && date && (
        <button
          ...
          style={{
            ...
            background: showOverlay ? "rgba(0,240,168,0.15)" : "rgba(6,8,11,0.82)",
            border: "1px solid " + (showOverlay ? "rgba(0,240,168,0.5)" : "var(--border)"),
            borderRadius: 6, padding: "5px 10px", backdropFilter: "blur(8px)",
            fontFamily: "var(--mono)", fontSize: 10, color: showOverlay ? "#00f0a8" : "var(--text2)",
