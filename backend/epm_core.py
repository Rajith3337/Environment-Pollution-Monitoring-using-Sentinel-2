...
KMEANS_N_INIT      = 10         # independent KMeans restarts (best inertia kept)
KMEANS_MAX_ITER    = 300        # max iterations per run

# Retry wait schedule in seconds (one entry per retry attempt, 0-based)
RETRY_DELAYS = [10, 10, 20, 30, 60]
...
def choose_parallelism(total_pixels: int, scene_count: int) -> tuple[int, int]:
    ...
    if total_pixels >= 20_000_000:
        ...
        band_workers = max(2, min(len(BANDS), MAX_TOTAL_HTTP // max(1, date_workers)))
...
def compute_raqi(indices):
    ...
    # Base features for evaluating environmental stress and quality
    eval_keys = ["NDTI", "NBR", "NDVI", "NDMI", "SAVI"]
    # We mathematically align vectors so that High value = High pollution risk
    invert_map = {"NDTI": 1, "NBR": -1, "NDVI": -1, "NDMI": -1, "SAVI": -1}

    H, W = indices["NDTI"].shape
    # Stack the selected environmental features
    stacked = []
    for k in eval_keys:
        arr = indices[k] * invert_map[k]
        stacked.append(arr)
        
    stacked_arr = np.stack(stacked, axis=-1)  # (H, W, 5)
    ...
    if n_valid > 5:
        X = stacked_arr[valid_mask]  # (n_valid, 5)
        # Standardize features for normalized PCA reduction
        mean_X = X.mean(axis=0)
        std_X = X.std(axis=0)
        std_X[std_X == 0] = 1.0  # prevent div by 0
        X_scaled = (X - mean_X) / std_X
        
        # Extract Maximum Variance explicitly across all independent indicators
        ...
        # Min-max scale the raw variance PC1 score strictly back to [0, 1] range for frontend parity
        pc1_min, pc1_max = np.percentile(pc1, 1), np.percentile(pc1, 99)
        if pc1_max == pc1_min:
            pc1_max = pc1_min + 1e-6
...
def compute_clusters(raqi):
    ...
    # ── Re-label so class 0 = lowest centroid (cleanest) ─────────────────
    raw_centroids  = gmm.means_.flatten()                   # (k,)
    order          = np.argsort(raw_centroids)              # ascending sort
    rank           = np.empty_like(order)
    rank[order]    = np.arange(KMEANS_N_CLUSTERS)           # old_label → new_label
    centroids_sorted = raw_centroids[order].tolist()

    log(f"  GMM centroids (sorted): {[f'{c:.4f}' for c in centroids_sorted]}", "CLUSTER")

    # ── Predict all valid pixels probabilistically ──────────────────────────────────
    raw_labels  = gmm.predict(valid_vals)                   # (n_valid,)
